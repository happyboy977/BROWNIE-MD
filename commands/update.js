    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const req = client.get(url, {
                headers: {
                    'User-Agent': 'BrownieMD-Updater/1.0',
                    'Accept': '*/*'
                }
            }, res => {
...
async function restartProcess(sock, chatId, message) {
    try {
        await sock.sendMessage(chatId, { text: '✅ Update complete! Restarting…', ...channelInfo }, { quoted: message });
    } catch {}
    try {
...
async function updateCommand(sock, chatId, message, zipOverride) {
    ...
    if (!message.key.fromMe && !isOwner) {
        await sock.sendMessage(chatId, { text: 'Only bot owner or sudo can use .update', ...channelInfo }, { quoted: message });
        return;
    }
    try {
        await sock.sendMessage(chatId, { text: '🔄 Updating the bot, please wait…', ...channelInfo }, { quoted: message });
        if (await hasGitRepo()) {
        ...
        try {
            await sock.sendMessage(chatId, { text: `✅ Update done. Restarting…`, ...channelInfo }, { quoted: message });
        } catch {
            await sock.sendMessage(chatId, { text: '✅ Restarted Successfully\nType .ping to check latest version.', ...channelInfo }, { quoted: message });
        }
        await restartProcess(sock, chatId, message);
    } catch (err) {
        console.error('Update failed:', err);
        await sock.sendMessage(chatId, { text: `❌ Update failed:\n${String(err.message || err)}`, ...channelInfo }, { quoted: message });
    }
}
