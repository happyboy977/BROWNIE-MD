...
const messageTemplates = {
    ...
    success: (imageUrl) => ({
        image: { url: imageUrl },
        caption: `GENERATED BY ${settings.botName || 'Brownie-MD'}`,
        ...channelInfo
    })
};
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await sock.sendMessage(chatId, messageTemplates.error("Invalid text generator type", quotedMsg), { quoted: quotedMsg });
            }
            ...
            await sock.sendMessage(chatId, messageTemplates.success(result.image), { quoted: quotedMsg });
        } catch (error) {
            console.error('Error in text generator:', error);
            await sock.sendMessage(chatId, messageTemplates.error(`Error: ${error.message}`, quotedMsg), { quoted: quotedMsg });
        }
    } catch (error) {
        console.error('Error in textmaker command:', error);
        await sock.sendMessage(chatId, messageTemplates.error("An error occurred. Please try again later.", quotedMsg), { quoted: quotedMsg });
    }
}
