...
if (!state.assetPath) {
await sock.sendMessage(chatId, { text: 'Hi', ...channelInfo }, { quoted: message });
return;
}
const assetPath = path.join(__dirname, '..', state.assetPath);
        ...
        try {
            if (state.type === 'sticker') {
                await sock.sendMessage(chatId, { sticker: fs.readFileSync(assetPath), ...channelInfo }, { quoted: message });
                return;
            }
...
if (!isOwner) return sock.sendMessage(chatId, { text: 'Only Owner or Sudo can use this command.', ...channelInfo }, { quoted: message });
const onoff = (args || '').trim().toLowerCase();
if (!onoff || !['on','off'].includes(onoff)) {
return sock.sendMessage(chatId, { text: 'Usage: .mention on|off', ...channelInfo }, { quoted: message });
}
const state = loadState();
state.enabled = onoff === 'on';
saveState(state);
return sock.sendMessage(chatId, { text: `Mention reply ${state.enabled ? 'enabled' : 'disabled'}.`, ...channelInfo }, { quoted: message });
}
...
if (!qMsg) return sock.sendMessage(chatId, { text: 'Reply to a message or media (sticker/image/video/audio/document).', ...channelInfo }, { quoted: message });
...
else return sock.sendMessage(chatId, { text: 'Unsupported. Reply to text/sticker/image/video/audio/document.', ...channelInfo }, { quoted: message });

if (type === 'text') {
buf = Buffer.from(qMsg.conversation || qMsg.extendedTextMessage?.text || '', 'utf8');
if (!buf.length) return sock.sendMessage(chatId, { text: 'Empty text.', ...channelInfo }, { quoted: message });
} else {
try {
...
return sock.sendMessage(chatId, { text: 'Failed to download media.', ...channelInfo }, { quoted: message });
}
}

if (buf.length > 1024 * 1024) {
return sock.sendMessage(chatId, { text: 'File too large. Max 1 MB.', ...channelInfo }, { quoted: message });
}
...
return sock.sendMessage(chatId, { text: 'Failed to save file.', ...channelInfo }, { quoted: message });
}
...
saveState(state);
return sock.sendMessage(chatId, { text: 'Mention reply media updated.', ...channelInfo }, { quoted: message });
}
